...
export default function App() {
  ...
  useEffect(() => { const prefs = loadPrefs(); if (!prefs) { setActiveSources([]); setSetupPrefs(null); setSetupOpen(true); } else { applyPrefs(prefs); void loadFeed(false, prefs.sources); } }, [applyPrefs, loadFeed]);
  ...
  return <div className="app-shell"><header className="app-header"><div className="brand-lockup"><img src="/brievox-logo.png" alt="AI News Aggregator logo" className="brand-logo" /><div><h1 className="brand-name">AI News Aggregator</h1><span>AI intelligence, curated daily</span></div></div><button className="icon-button" type="button" aria-label="Open settings" title="Configure sources" onClick={openSetup}><Settings aria-hidden="true" /></button></header><div className="app-layout"><aside className="sidebar"><div className="sidebar-label">Your sources</div><nav id="sidebar-nav" aria-label="News sources"><Sidebar activeSources={activeSources} filter={filter} onFilter={(next) => setFilter(next)} /></nav></aside><main className="content-area"><div className="content-heading"><div><p className="eyebrow">LATEST SIGNALS</p><h2>AI news worth your attention.</h2></div>{activeSources.length > 0 && <button className={`refresh-button${loading ? ' is-refreshing' : ''}`} type="button" aria-label="Refresh feed" title="Refresh feed" disabled={loading} onClick={() => void loadFeed(true)}><RefreshCw aria-hidden="true" /></button>}</div><div id="news-grid" className="news-grid">{loading ? <LoadingState total={activeSources.length} progress={progress} /> : error ? <p className="error-message">Failed to load news.</p> : !activeSources.length ? <EmptyState onChoose={openSetup} /> : !filteredArticles.length ? <p className="empty-state">No articles found.</p> : filteredArticles.map((article) => <NewsCard article={article} key={article.id} />)}</div></main></div>{setupOpen && <SetupModal initialPrefs={setupPrefs} onComplete={handleComplete} onClose={() => setSetupOpen(false)} />}</div>;
}
