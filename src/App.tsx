...
function LoadingState({ total, progress, showSkeletons }: { total: number; progress: FeedProgress | null; showSkeletons: boolean }) {
  const completed = Math.min(progress?.completed || 0, total);
  return <><div className="feed-loading" role="status" aria-live="polite"><div className="feed-loading-label"><span>Loading sources</span><span>{completed}/{total}</span></div><div className="feed-progress" role="progressbar" aria-label="News loading progress" aria-valuemin={0} aria-valuemax={total} aria-valuenow={completed}><span style={{ width: `${total ? (completed / total) * 100 : 100}%` }} /></div></div>{showSkeletons && Array.from({ length: 6 }, (_, index) => <div className="news-card skeleton-card" style={{ animationDelay: `${index * 0.08}s` }} key={index}><div className="skeleton-line skeleton-title" /><div className="skeleton-line" /><div className="skeleton-line short" /><div className="skeleton-footer" /></div>)}</>;
}
...
export default function App() {
  ...
  const loadFeed = useCallback(async (forceRefresh = false, sources = activeSources) => {
    ...
    setArticles([]);
    try { setArticles(await fetchAllFeeds({ forceRefresh, selectedSourceIds: sources, onProgress: setProgress, onArticles: setArticles })); } catch { setError(true); } finally { setLoading(false); }
  }, [activeSources]);
  ...
  return <div className="app-shell"><header className="app-header"><div className="brand-lockup"><img src="/ai-news-aggregator-logo.webp" alt="AI News Aggregator logo" className="brand-logo" /><div><h1 className="brand-name">AI News Aggregator</h1><span>AI intelligence, curated daily</span></div></div><button className="icon-button" type="button" aria-label="Open settings" title="Configure sources" onClick={openSetup}><Settings aria-hidden="true" /></button></header><div className="app-layout"><aside className="sidebar"><div className="sidebar-label">Your sources</div><nav id="sidebar-nav" aria-label="News sources"><Sidebar activeSources={activeSources} filter={filter} onFilter={(next) => setFilter(next)} /></nav></aside><main className="content-area"><div className="content-heading"><div><p className="eyebrow">LATEST SIGNALS</p><h2>AI news worth your attention.</h2></div>{activeSources.length > 0 && <button className={`refresh-button${loading ? ' is-refreshing' : ''}`} type="button" aria-label="Refresh feed" title="Refresh feed" disabled={loading} onClick={() => void loadFeed(true)}><RefreshCw aria-hidden="true" /></button>}</div><div id="news-grid" className="news-grid">{loading ? <><LoadingState total={activeSources.length} progress={progress} showSkeletons={!articles.length} />{articles.map((article) => <NewsCard article={article} key={article.id} />)}</> : error ? <p className="error-message">Failed to load news.</p> : !activeSources.length ? <EmptyState onChoose={openSetup} /> : !filteredArticles.length ? <p className="empty-state">No articles found.</p> : filteredArticles.map((article) => <NewsCard article={article} key={article.id} />)}</div></main></div>{setupOpen && <SetupModal initialPrefs={setupPrefs} onComplete={handleComplete} onClose={() => setSetupOpen(false)} />}</div>;
}
