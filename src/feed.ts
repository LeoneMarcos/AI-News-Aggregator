...
function normalizeFeedItem(item: RawFeedItem, source: FeedSource): Article {
  ...
  return {
    ...
    description:
      rawDescription.substring(0, 600) +
...
function parseXmlEntry(entry: Element, source: FeedSource): Article | null {
  ...
  const contentEncoded =
    xmlText(entry, "content:encoded") || xmlText(entry, "content");
  const description =
    contentEncoded ||
  ...
  return normalizeFeedItem(
    {
      ...
      author:
        xmlText(entry, "dc:creator") || xmlText(entry, "author") || source.name,
...
function parseRssXml(xmlTextValue: string, source: FeedSource): Article[] {
  ...
  if (entries.length === 0)
    entries = Array.from(document.querySelectorAll("entry"));
  ...
  try {
    ...
    if (proxyUrl.includes("rss2json.com")) {
      ...
      if (data.status !== "ok" || !data.items?.length)
        throw new Error("RSS JSON response was empty");
...
async function fetchSingleFeed(source: FeedSource): Promise<FeedResult> {
  ...
  try {
    const items = await Promise.any(
      CORS_PROXIES.map((makeProxyUrl, index) =>
        fetchProxy(makeProxyUrl, source, controllers[index]),
      ),
    );
...
function filterByHours(articles: Article[], hours: number): Article[] {
  const filtered = articles.filter((article) =>
    isWithinHours(article.pubDate, hours),
  );
  return filtered.length === 0 && articles.length > 0
    ? articles.slice(0, 30)
    : filtered;
}
...
export async function fetchAllFeeds({
  ...
  const notifyProgress = (progress: FeedProgress) => {
    try {
      ...
      /* Progress UI must not interrupt loading. */
    }
  };
  ...
  if (!forceRefresh) {
    ...
    if (cached) {
      ...
      notifyProgress({
        completed: activeSources.length,
  ...
  const notifyArticles = () => {
    try {
      ...
      /* Progressive rendering must not interrupt loading. */
    }
  };
  ...
  const feedResults = results.flatMap((result) =>
  ...
  const allSourcesFailed = results.every(
    (result) =>
