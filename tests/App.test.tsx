...
describe("App", () => {
  ...
  it("renders the React shell and empty feed state without preferences", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "AI News Aggregator" }),
    ).toBeTruthy();
    expect(screen.getByText("Choose your news sources.")).toBeTruthy();
    expect(
      screen.getByRole("dialog", { name: "AI News Aggregator setup" }),
    ).toBeTruthy();
  });
  ...
  it("loads saved preferences without reinitializing the feed effect", async () => {
    localStorage.setItem(
      ...
      JSON.stringify({ sources: ["techcrunch"] }),
    );
    localStorage.setItem(
      "ai_news_aggregator_feed_cache_techcrunch",
    ...
    expect(
      screen.queryByRole("dialog", { name: "AI News Aggregator setup" }),
    ).toBeNull();
  });
  ...
  it("finishes the setup modal exit within the short animation window", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Close source settings" }),
    );
    expect(
      screen.getByRole("dialog", { name: "AI News Aggregator setup" }),
    ).toBeTruthy();
