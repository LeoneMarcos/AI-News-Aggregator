...
describe("feed.js", () => {
  ...
  it("should handle invalid XML gracefully", async () => {
    ...
    await expect(fetchAllFeeds({ forceRefresh: true })).rejects.toThrow(
      "All selected news sources failed to load.",
    );
  });
  ...
  it("should ignore expired cache", async () => {
    ...
    await expect(fetchAllFeeds()).rejects.toThrow(
      "All selected news sources failed to load.",
    );
    // Cache key now includes sorted source IDs
    const expectedKey = `ai_news_aggregator_feed_cache_${SOURCES.map(
      (s) => s.id,
    )
      .sort()
      .join(",")}`;
  ...
  it("should report progress once for each loaded source", async () => {
    ...
    try {
      ...
      expect(
        progress.every(
          (event) => event.total === 3 && event.status === "loaded",
        ),
      ).toBe(true);
  ...
  it("should complete progress when a source fails", async () => {
    ...
    try {
      ...
      ).rejects.toThrow("All selected news sources failed to load.");
      expect(progress).toEqual([
        { completed: 1, total: 1, sourceId: "failed", status: "failed" },
      ]);
    } finally {
      SOURCES.length = 0;
      SOURCES.push(...originalSources);
  ...
  it("should report cached sources as completed", async () => {
    ...
    try {
      await fetchAllFeeds({
        selectedSourceIds: ["src-a", "src-b"],
        onProgress: (event) => progress.push(event),
      });

      expect(progress).toEqual([
        { completed: 2, total: 2, sourceId: null, status: "cached" },
      ]);
